...
def plot_ellipse_simple(p1, p2, fisher_matrix, cosmo_params, axs=None, color=None, label=None, dash=False, shade=False):
    ...
    if axs is None:
        ...
        # axs[0,0].set_ylim(0, max(norm.pdf(x, mu, sig)))

        axs[1,1].set_xlim(p2_min, p2_max)
        mu = center[1]
        sig = stdv[1]
        x = np.linspace(mu - 10 * sig, mu + 10 * sig, 100)
        # axs[1,1].set_ylim(0, max(norm.pdf(x, mu, sig)))
